#!/usr/bin/env python
"""
CLI Entry Point for Activation-Based Structured Pruning of ViT models.

Usage:
    # MLP pruning:
    python vit_prune/run_prune.py --model vit_base_patch16_224 --target mlp --sparsity 0.3

    # Attention head pruning:
    python vit_prune/run_prune.py --model vit_base_patch16_224 --target attn --sparsity 0.25

    # Q/K dimension pruning with Sylvester-based logit-matching compensation:
    python vit_prune/run_prune.py --model vit_base_patch16_224 --target attn \
        --attn_mode dim-logit --qk_sparsity 0.3 --covariance_mode exact

    # ViT Large:
    python vit_prune/run_prune.py --model vit_large_patch16_224 --target both --sparsity 0.3
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Activation-based structured pruning for ViT models'
    )
    ...
    parser.add_argument(
        ...
        help='Fraction of Q/K dimensions to prune per head (for dim-logit mode). '
             'Uses E[q^2 * k^2] joint energy score for ranking.'
    )
    parser.add_argument(
        '--min_qk_dim', type=int, default=8,
        help='Minimum Q/K dimensions to keep per head (for dim-logit mode)'
    )
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Activation-Based Structured Pruning (ViT)")
    logger.info("=" * 60)
