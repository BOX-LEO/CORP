...
def parse_args():
    parser = argparse.ArgumentParser(description='Analyze activation redundancy for DINOv2')
    parser.add_argument('--model', type=str, default='huge')
    parser.add_argument('--nyu_path', type=str, default=None)
    parser.add_argument('--use_ade', action='store_true', default=True)
    parser.add_argument('--no_ade', action='store_true', help='Disable ADE20k data')
    parser.add_argument('--nyu_samples', type=int, default=1000)
    parser.add_argument('--ade_samples', type=int, default=1000)
...
def main():
    ...
    if args.output_file is None:
        ...
        output_file = output_dir / f"redundancy_analysis_dinov2_{args.model}_{timestamp}.json"
    else:
        output_file = Path(args.output_file)

    logger.info(f"Model: dinov2_{args.model}")
    logger.info(f"Output file: {output_file}")

    # Load model
    from dino_prune.models import load_model
    ...
    from dino_prune._datasets import create_calibration_loader
    ...
    # Get hook points for MLP activations (auto-handles SwiGLU for huge)
    hook_points = get_hook_points(model, 'mlp')
    collector.register_hooks(hook_points)
    logger.info(f"Registered hooks on {len(hook_points)} layers: {hook_points[:2]}...")
